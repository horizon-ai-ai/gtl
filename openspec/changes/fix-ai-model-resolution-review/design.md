...
4. **#5 scope boundary.** In scope: the conversation request-path siblings — conversation intent classification (`intent-resolver`) and `generateSiteSchema` (`site-builder`) accept and use a resolved `providerConfig`. Out of scope but enumerated: standalone endpoints `support/ask`, `admin/copilot`, and `website-builder/orchestrator` remain env-based; this change documents them as a follow-up rather than migrating them, to keep the task count bounded.
