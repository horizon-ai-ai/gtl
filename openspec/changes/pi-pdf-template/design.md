...
- The rendered PDF is a single A4 page (595×842 pt) laid out as: a title row, a seller block (top-left) and a customer block (below or beside) sourced from `CompanyProfile`, a shipping details box, a line items table with one row from the inquiry, a totals breakdown, an additional details block, and a signature block.
...
- `PIData` (in `src/lib/pdf/pi-data.ts`) is a TypeScript interface with fields for: invoice metadata (invoice number, dates, customer id), seller block, customer block, shipping details, line items (array — currently length 1), totals breakdown, additional details, signature. All fields are typed as `string` (already-formatted display values) so the renderer does no formatting.
